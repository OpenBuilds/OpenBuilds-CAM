...
function IconLib_Pagination(WorkingData, page = 1){
  ...
  var template = IconLib_Grid_Sty_Template();
  ...
  $('#iconlib_PageOf').closest('div').find('.append').text(`${IconsPageNumber} Of ${totPages}`);
...
async function loadIconItem(id){
  arrIndex = IconsMasterData.map(function(x) {return x.uid;}).indexOf(id);
...
function IconLib_Grid_Sty_Template(){
  var template = `
  <style>
    .image {
      opacity: 1;
    ...
    .iconThumbnailImage{
      position: relative; top: 50%; transform: perspective(1px) translateY(-50%); max-width: 40px; max-height: 40px;
    }
